...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  case baron:
    return baronCard(currentPlayer, handPos, state, choice1);
  case great_hall:
    return great_hallCard(currentPlayer, handPos, state);
  ...
  case ambassador:
    return amabsadorCard(currentPlayer, handPos, state);
  ...
  case sea_hag:
    for (i = 0; i < state->numPlayers; i++)
    {
      if (i != currentPlayer)
      {
        ...
        printf("%d:%d",state->deckCount[i] ,state->deck[i][state->deckCount[i]--]);  
      }
    }
    return 0;
...
int adventurerCard(int currentPlayer, int handPos, struct gameState *state, int drawntreasure, int cardDrawn, int *temphand)
{
int z=0;
...
int amabsadorCard(int currentPlayer, int handPos, struct gameState *state, int cardPosToSupply, int numCopysToSupply)
{
  int j = 1; //used to check if player has enough cards to discard

  if (numCopysToSupply > 2 || numCopysToSupply < 0)
  {
    return -1;
  }

  if (cardPosToSupply == handPos)
  {
    return -1;
  }
  ...
  {
    if (i != handPos && i == state->hand[currentPlayer][cardPosToSupply] && i != cardPosToSupply)
    {
      j++;
    }
  }
  if (j < numCopysToSupply)
  {
    return -1;
  }

  if (DEBUG)
    printf("Player %d reveals card number: %d\n", currentPlayer, state->hand[currentPlayer][cardPosToSupply]);

  //increase supply count for choosen card by amount being discarded
  state->supplyCount[state->hand[currentPlayer][cardPosToSupply]] += numCopysToSupply;
  ...
  for (int i = 0; i < state->numPlayers; i++)
  {
    if (i != currentPlayer)
    {
      gainCard(state->hand[currentPlayer][cardPosToSupply], state, 0, i);
    }
  }
  ...
  for (j = 0; j < numCopysToSupply; j++)
  {
    for (int i = 0; i < state->handCount[currentPlayer]; i++)
    {
      if (state->hand[currentPlayer][j] == state->hand[currentPlayer][cardPosToSupply])
      {
...
int baronCard(int currentPlayer, int handPos, struct gameState *state, int discardEstate)
{
    ...
    while (card_not_discarded)
    {
      ...
      else if (p > state->handCount[currentPlayer])
      {
        ...
        if (supplyCount(estate, state) > 0)
        {
          ...
          state->supplyCount[estate]=-1; //Decrement estates
          if (supplyCount(estate, state) == 0)
          {
            isGameOver(state);
          }
